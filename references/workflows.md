...
- After that, the LLM should read the bundle and author the final `DESIGN.md`, `design-spec.html`, and `app-preview.html`.
...
Default output rule:
- If the user asks to generate or create a design system and does not specify a format, first generate the intermediate bundle, then author `design-spec.html`, `DESIGN.md`, and `app-preview.html` from that bundle.
...
- `app-preview.html` should be generated by default for design-system runs, and if the user explicitly asks for both artifact types, `design-spec.html` must still come from the template while `app-preview.html` remains the separate product mockup.
...
- `design-system` should usually end by generating `design-spec.html`, `DESIGN.md`, and `app-preview.html` unless the user asked for a different deliverable.
