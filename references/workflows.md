...
- `scripts/generate.py` should write `generation-bundle.json` and `manifest.json`, then `scripts/render_artifacts.py` should render `DESIGN.md` and `design-spec.html`.
- The LLM may inspect or correct the bundle inputs, but it should not directly author `DESIGN.md` or `design-spec.html`.
- After `DESIGN.md` is finalized, `app-preview.html` should be authored last by the LLM from the product requirements plus that document.
...
- After rendering the spec artifacts, run `scripts/finalize_manifest.py <run_dir>` so the manifest moves from `bundle-generated` to `spec-artifacts-finalized`, or to `preview-artifact-recorded` if `app-preview.html` already exists.

Default output rule:
- If the user asks to generate or create a design system and does not specify a format, first generate the intermediate bundle, then render `design-spec.html` and `DESIGN.md` from that bundle.
- If the user also wants a product mockup, generate `app-preview.html` after `DESIGN.md` is finalized.
...
- If the user explicitly asks for both artifact types, `design-spec.html` must still come from the template while `app-preview.html` is authored separately from the product requirements plus `DESIGN.md`.

Script role reminder:
- `scripts/generate.py` is the orchestrator and recommended entry point for full script-flow generation.
...
- `design-system` should usually end by generating `design-spec.html` and `DESIGN.md`, then authoring `app-preview.html` from the product requirements plus `DESIGN.md` unless the user asked for a different deliverable.
